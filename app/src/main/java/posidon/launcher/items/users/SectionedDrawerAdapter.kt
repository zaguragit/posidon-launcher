...
class SectionedDrawerAdapter : BaseAdapter(), SectionIndexer, HighlightAdapter {
    ...
    override fun getCount(): Int = Home.appSections.size
    override fun getItem(i: Int) = Home.appSections[i]
    override fun getItemId(i: Int): Long = 0

    override fun getView(i: Int, cv: View?, parent: ViewGroup): View? {
        val section = Home.appSections[i]
    ...
    override fun getSectionForPosition(i: Int): Int = savedSections.indexOf(Home.appSections[i][0].label!![0].toUpperCase())
    override fun getPositionForSection(i: Int): Int {
        val char = savedSections[i]
        for (j in Home.appSections.indices) {
            if (Home.appSections[j][0].label!![0].toUpperCase() == char) {
                return j
            }
        }
        return 0
