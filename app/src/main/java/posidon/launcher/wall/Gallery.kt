...
class Gallery : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.gallery).setPadding(gridsidepadding, getStatusBarHeight(this) + Tools.dp(this, 4).toInt(), gridsidepadding, toolbarHeight + Tools.dp(this, 20).toInt())
        findViewById<View>(R.id.pickwallbtn).setOnClickListener {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    pickwall()
                } else AlertDialog.Builder(this@Gallery)
                        .setTitle("Permission needed")
                        .setMessage("posidon launcher needs the permission to read storage to access the images on your device")
                        .setPositiveButton("OK") { _, _ -> requestPermissions(arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE), 2) }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        if (requestCode == 2 && grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) pickwall()
